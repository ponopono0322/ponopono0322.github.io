...
- 적절한 구성 변경
- 리소스 공유
가 있다. `LiveData`에서 `setValue()`와 `postValue()`는 `protected` 되어있어 외부에서 클래스 내의 값을 변경해줄 수 없다(*get* 만 가능). 따라서 `LiveData`를 상속받은 `MutableLiveData`를 사용한다(*set/get* 모두 가능).
...
## 6. `object` 와 `companion object`의 차이?
...
## 9. 코틀린의 `class deault`가 `final`인 이유?
